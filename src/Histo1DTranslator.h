...
#ifndef HISTO1DTRANSLATOR
#define HISTO1DTRANSLATOR 1
...
class Histo1DTranslator : public Translator {

public: 
  // constructor for writing to XML
  Histo1DTranslator(const AIDA::IHistogram1D * histo, const std::string & name, const std::string & path ); 
  // constructor for reading from XML
  Histo1DTranslator(const DataXML::DataObject * element); 

  virtual ~Histo1DTranslator(); 

private:
// usually copying is non trivial, so we make this unaccessible
  Histo1DTranslator(const Histo1DTranslator &); 
  Histo1DTranslator & operator = (const Histo1DTranslator &); 
...
public: 
  ...
  AIDA::Dev::IDevHistogram1D * createFromXML(AIDA::Dev::IDevHistogramFactory& factory); 

  /// getters for class members 
  DataXML::DataObject & dataObject() { return m_element; } 
  const AIDA::IHistogram1D * histo() { return m_histo; } 
...
private: 
  ...
  const AIDA::IHistogram1D * m_histo;  
  /// object name 
  std::string m_name; 
  /// object path 
  std::string m_path;  
