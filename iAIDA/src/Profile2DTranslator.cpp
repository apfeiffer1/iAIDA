...
// Implementation file for class Profile2DTranslator
// 
...
iAIDA::AIDA_XMLStore::Profile2DTranslator::Profile2DTranslator(const AIDA::IProfile2D * histo, const std::string & name, const std::string & path ) : 
  m_histo(histo), m_name(name), m_path(path)
{
}
/// constructors for reading (xml -> histo) 
iAIDA::AIDA_XMLStore::Profile2DTranslator::Profile2DTranslator(const DataXML::DataObject * xmlObject) : 
  m_element(*xmlObject)
{
}


iAIDA::AIDA_XMLStore::Profile2DTranslator::~Profile2DTranslator() 
{  // no op
}

iAIDA::AIDA_XMLStore::Profile2DTranslator::Profile2DTranslator(const iAIDA::AIDA_XMLStore::Profile2DTranslator &) : Translator()  
{
}

iAIDA::AIDA_XMLStore::Profile2DTranslator & iAIDA::AIDA_XMLStore::Profile2DTranslator::operator = (const iAIDA::AIDA_XMLStore::Profile2DTranslator &rhs) 
{
...
bool iAIDA::AIDA_XMLStore::Profile2DTranslator::toXML() 
{ 
  // write name and title
  appendObjectHeader(m_element,"profile2d",m_name,m_histo->title(),m_path); 
...
bool iAIDA::AIDA_XMLStore::Profile2DTranslator::setAxes()
{ 
...
bool iAIDA::AIDA_XMLStore::Profile2DTranslator::setStatistics()
{ 
...
bool iAIDA::AIDA_XMLStore::Profile2DTranslator::setData()
{ 
  ...
  for (int i = -2; i < nbx; ++i) {
    for (int j = -2; j < nby; ++j ) { 
      ...
      double rms     =  m_histo->binRms     (i,j);
      appendHisto2DBinData(dataElement, i, j, entries, height, error, binMeanX, binMeanY,rms);
    }
  }
...
AIDA::Dev::IDevProfile2D * iAIDA::AIDA_XMLStore::Profile2DTranslator::createFromXML(AIDA::Dev::IDevHistogramFactory& factory)
{ 
  // read header 
  ...
  AIDA::Dev::IDevProfile2D * h = 0; 
  if (edgesX.empty() )  
    // equal bins histograms
    h = factory.createProfile2D( title, numberOfBinsX, lowerEdgeX, upperEdgeX, numberOfBinsY, lowerEdgeY, upperEdgeY );
  else   
    // variable bins histograms 
    h = factory.createProfile2D( title, edgesX, edgesY, emptyString );
  ...
  for (std::vector<DataXML::DataObject>::const_iterator binElement = dataElement->children().begin(); binElement != dataElement->children().end(); ++binElement) {  
    if (binElement->name() == "bin2d") {
      ...
      h->setBinContents(xbin,ybin,entries, height, error, rms, wMeanX, wMeanY);
    }
  }
