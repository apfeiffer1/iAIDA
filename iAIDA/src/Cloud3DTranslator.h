...
// Header file for class Cloud3DTranslator
// 
...
/*    
 *    class containing methods to transfrom a 3D Cloud to and from XML 
 *    using DataXML library.
 *    When translating to XML the class it is constructed using 
 *    AIDA Cloud3D plus name and path and creates a DataXML::DataObject. 
...
#ifndef CLOUD3DTRANSLATOR
#define CLOUD3DTRANSLATOR 1
...
class Cloud3DTranslator : public Translator {

public: 
  // constructor for writing
  Cloud3DTranslator(const AIDA::ICloud3D * cloud, const std::string & name, const std::string & path ); 
  // constructor for reading 
  Cloud3DTranslator(const DataXML::DataObject * element); 

  virtual ~Cloud3DTranslator(); 

private:
// usually copying is non trivial, so we make this unaccessible
  Cloud3DTranslator(const Cloud3DTranslator &); 
  Cloud3DTranslator & operator = (const Cloud3DTranslator &); 

public: 


  DataXML::DataObject & dataObject() { return m_element; } 
  const AIDA::ICloud3D * histo() { return m_cloud; } 
  ...
  AIDA::Dev::IDevCloud3D * createFromXML(AIDA::Dev::IDevHistogramFactory& factory); 
